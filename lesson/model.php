...
/*
	public function getImageList($conditions = array(), $image_type = 'shop_thumbnail') {
		...
		$sql = "SELECT * FROM ".$this->getWPTableName('postmeta');
		$where = array('meta_key' => '_wp_attachment_metadata');
		$sql.= $this->getSQLWhere(array_merge($where, $conditions));
		...
		foreach($aRowset as $row) {
			...
			foreach($image_type as $type) {
				$aThumbs[$row['post_id']][$type] = (isset($a['sizes'][$type]['file'])) ? UPLOADS_DIR.$dir.'/'.$subdir.'/'.$a['sizes'][$type]['file'] : UPLOADS_DIR.$a['file'];
			}
		}
		return $aThumbs;
	}
